...
namespace DotNet
{
    public class  RandomSolver
    {
        ...
        private IEnumerable<Package> _normalPackages { get { return _packages.Where(p => p.WeightClass != 2); } }
        private IEnumerable<Package> _heavyPackages { get { return _packages.Where(p => p.WeightClass == 2); } }
        ...
        public RandomSolver(List<Package> packages, Vehicle vehicle)
        {
            _packages = packages;
        ...
        public List<PointPackage> Solve()
        {
            ...
            while (_placedPackages.Count < _packages.Count)
            {
                Package package;
        ...
        private void SortByMaxArea()
        {
            ...
            _packages = _packages.OrderByDescending(x => x.Width * x.Length).ToList();
        }
        ...
        private Package GetNextHeavyPackage()
        {
            ...
            var list = _heavyPackages.Where(p => !_placedPackages.Contains(p)).ToList();
        ...
        private Package GetNextNormalPackage()
        {
            ...
            var list = _normalPackages.Where(p => !_placedPackages.Contains(p)).ToList();
            var index = _random.Next(0, list.Count);
            var item = list.ElementAt(index);
            return item;
        }
